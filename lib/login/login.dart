...
class _LoginState extends State<Login> {
   ...
   Future login(String email, String pass) async {
     ...
     try {
       await _auth.signInWithEmailAndPassword(email: email, password: passMD5)
           .then((value) async {
           ...
           await getData(value.user!.uid).whenComplete(() async {
             ...
             pref.setStringList('login', [value.user!.uid, name, no, email]);
              ...
              Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) =>
                 HomeScreenStaff(uid: value.user!.uid, name: name, no: no, email: email,)), (route) => false);
       ...
       switch(error.code){
         case 'user-not-found': showToast('Tài khoản không tồn tại :(((', null);
         break;
         case 'wrong-password': showToast('Mật khẩu sai', null);
         break;
         default: showToast(error.code, null);
       }
     }
   }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Form(
                      child: Column(
                        children: [
                          Rounded_Input(icon: Icon(Icons.email_rounded), hintText: 'Email', textController: _emailController,),
                          addVerticalSpace(DefaultPadding),
                          Rounded_Input(icon: Icon(Icons.lock), hintText: 'Mật khẩu', textController: _passController, hide: true,),
                          addVerticalSpace(DefaultPadding),
                        ],
                      ),
                    ),
                    InkWell(
                      onTap: () {
                        ...
                        if(_emailController.text.trim() != ''){
                          ...
                          if(_passController.text.trim() != '') {
                            ...
                            login(_emailController.text.trim(), _passController.text.trim()).whenComplete((){
                              setState(() {
                                isLoading = false;
                              });
                            });
                          ...
                          }else showToast('Vui lòng nhập mật khẩu', null);
                        }else{
                          if(_passController.text.trim() != '')
                      ...
                      child: Container(
                        ...
                        child: Center(
                            child: isLoading != true ? Text('ĐĂNG NHẬP', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w700))
                                : SizedBox(
                                width: 20,
                                height: 20,
                    ...
                    SizedBox(
                        ...
                        child: Text('Sử dụng tài khoản và mật khẩu do ADMIN cung cấp để đăng nhập', style: TextStyle(fontSize: size.width * 0.038, color: Colors.grey), textAlign: TextAlign.center,)
                    ),
                  ],
                ),
              ),
