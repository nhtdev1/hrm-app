...
class NavigationDrawerWidget extends StatelessWidget{
  ...
  const NavigationDrawerWidget({Key? key, this.uid, this.name, this.no, this.email}) : super(key: key);
    ...
    return Container(
      ...
      child: Drawer(
        child: Material(
          ...
          child: ListView(
            children: <Widget>[
              buildHeader(name: _name, id: _no, width: ndw_width, email: _email,
                  onClicked: (){
                    Navigator.push(context, MaterialPageRoute(builder: (context)
                    => UserPage(name: _name, email: _email,)));
                  }),
              addLine(0),
              if (_name != 'ADMIN')
                buildMenuItem(text: 'Hồ sơ', icon: Icons.person, onClicked: (){
                  Navigator.pop(context);
                  Navigator.push(context, MaterialPageRoute(builder: (context)
                  => MemberDetails(uid: uid!)));
                }),
              // if (_name == 'ADMIN')
              //   buildMenuItem(text: 'Test Login', icon: Icons.login, onClicked: (){
              //     Navigator.pop(context);
              //   }),
              buildMenuItem(text: 'Đăng xuất', icon: Icons.logout, onClicked: (){
                Navigator.pop(context);
                showDialog<String>(
                    context: context,
                    builder: (BuildContext context) => AlertDialog(
                      ...
                      content: Text('Đăng xuất không?', style: TextStyle(fontSize: 18),),
                      actions: <Widget>[
                        TextButton(
                          onPressed: () async {
              ...
              buildMenuItem(text: 'Đổi mật khẩu', icon: Icons.lock_outline, onClicked: (){
                Navigator.push(context, MaterialPageRoute(builder: (context)
                => uid != null ? ChangePassword(uid: uid!) : ChangePassword(uid: 'admin')));
              }),
              buildMenuItem(text: 'Thông tin ứng dụng', icon: Icons.info_outline, onClicked: (){
                Navigator.push(context, MaterialPageRoute(builder: (context)
                => AppInfo()));
              }),
