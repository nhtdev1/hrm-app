...
class ScheduleStaff extends StatefulWidget {
  ...
  const ScheduleStaff({Key? key, required this.uid}) : super(key: key);
  @override
  _Schedule createState() => _Schedule();
}

class _Schedule extends State<ScheduleStaff> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Sắp Xếp', style: titleAppBar),
        iconTheme: IconThemeData(color: COLOR_PRIMARY_1),
        centerTitle: true,
      ),
      body: Column(
        children: [
          HeaderScheduleStaff(size, themeData),
          StreamBuilder(
  ...
  Container HeaderScheduleStaff(Size size, ThemeData themeData) {
    return Container(
